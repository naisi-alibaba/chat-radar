...
export function saveVerdict(db, v, syncedAt) {
  db.prepare(
    `INSERT INTO verdicts
      (chat_id, chat_name, category, hot, headline, reason, suggested_action, related, key_people, time_bucket, avatar, topic, latest_time, mention_count, synced_at)
     VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
     ON CONFLICT(chat_id) DO UPDATE SET
       ...
       topic=excluded.topic, latest_time=excluded.latest_time, mention_count=excluded.mention_count, synced_at=excluded.synced_at`,
  ).run(
    ...
    v.latest_time ?? '', v.mention_count ?? 0, syncedAt,
  )
}
